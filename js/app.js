...
function loadSettings() {
  try {
    const r = localStorage.getItem('smart_tran_2026');
    if (r) Object.assign(smartSettings, JSON.parse(r));
  } catch (_) {}
}
function saveSettings() {
  try {
    localStorage.setItem('smart_tran_2026', JSON.stringify(smartSettings));
  } catch (_) {}
}
...
function renderTabellaArticoli() {
  ...
  body.querySelectorAll('.btn-up,.btn-down').forEach((btn) =>
    btn.addEventListener('click', () => {
      ...
      [articoliAggiunti[idx], articoliAggiunti[dest]] = [
        articoliAggiunti[dest],
        articoliAggiunti[idx]
      ];
      renderTabellaArticoli();
    })
  );
...
function aggiornaBadgePreventivo() {
  aggiornaCalcoliRighe();
}

function aggiornaTotali() {
  const card = $id('totaliCard'),
    el = $id('totaleGenerale');
  if (!el) return;
  if (!articoliAggiunti.length) {
...
function aggiungiManuale() {
  const codice = $val('manCodice').trim(),
    descrizione = $val('manDescrizione').trim();
...
async function ripristinaPreventivo() {
  try {
    ...
    refreshTranLinkSelect();
    showToast(
      '🔄 Preventivo precedente ripristinato (' + articoliAggiunti.length + ' articoli)',
      3500
    );
  } catch (_) {}
}
...
function bindSmartControls() {
  ...
  if (elAC) elAC.checked = autoCosti;
  const onChange = () => {
...
function refreshTranLinkSelect() {
  ...
  $setText(
    'tranLinkInfo',
    articoliAggiunti.length ? '' : 'Aggiungi prima articoli nel tab Preventivo.'
  );
}
...
function applyTranArticleData(tranArt, selItems) {
  ...
  if (!$id('tranRegion')?.value) msgs.push('👉 Seleziona la Regione per calcolare');
  else if (svcToSet === 'GROUPAGE' && !$id('tranProvince')?.value)
    msgs.push('👉 Seleziona la Provincia per calcolare');
  else msgs.push('👉 Premi Calcola Trasporto');
...
function applyTranFromArticolo_byIdx(idx) {
  ...
  applyTranArticleData(
    tranArt || {
      pack: { palletType: listino.find((i) => i.codice === a.codice)?.palletType || '' }
    },
    [a]
  );
...
function addTranToPreventivo() {
  if (!lastTranResult || !Number.isFinite(lastTranResult.costCliente)) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  $id('btnLoadSavedCSV')?.addEventListener('click', async () => {
    ...
    aggiornaListinoSelect();
    updateListinoStats();
    showToast('✅ Listino: ' + listino.length + ' art.');
  });
